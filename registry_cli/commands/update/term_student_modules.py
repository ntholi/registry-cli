...
def update_specific_students_modules(
    db: Session, std_nos: List[int], term: str, reset: bool = False
) -> None:
    progress_file_suffix = "_".join(
        map(str, sorted(std_nos)[:5])
    )  # Use first 5 student numbers for filename
    if len(std_nos) > 5:
        progress_file_suffix += f"_plus_{len(std_nos) - 5}_more"

    sanitized_term = term.replace("/", "-").replace("\\", "-")
    progress_file = f"specific_students_update_progress_{sanitized_term}_{progress_file_suffix}.json"
    ...
    if progress["term"] != term or progress["student_numbers"] != std_nos:
        progress["term"] = term
        progress["student_numbers"] = std_nos
        progress["completed_students"] = []
        progress["failed_students"] = []
        progress["current_index"] = 0
        progress["total_students"] = len(std_nos)

    click.echo(f"Checking specified students have semesters in term {term}...")

    student_semesters = (
        db.query(StudentSemester)
        .join(StudentProgram)
        .filter(StudentSemester.term == term, StudentProgram.std_no.in_(std_nos))
        .order_by(StudentProgram.std_no)
        .all()
    )

    existing_student_numbers = [
        semester.program.std_no for semester in student_semesters
    ]
    missing_students = [
        std_no for std_no in std_nos if std_no not in existing_student_numbers
    ]

    if missing_students:
        click.secho(
            f"Warning: {len(missing_students)} students not found in term {term}:",
            fg="yellow",
        )
        for std_no in missing_students:
            click.echo(f"  - {std_no}")
        click.echo()

    if not existing_student_numbers:
        click.secho(
            f"No specified students found with semesters in term {term}", fg="red"
        )
        return
    ...
    remaining_students = [
        std_no
        for std_no in existing_student_numbers
        if std_no not in completed_students and std_no not in failed_students
    ]

    retry_students = [
        std_no for std_no in existing_student_numbers if std_no in failed_students
    ]

    all_students_to_process = retry_students + remaining_students

    click.echo(
        f"Found {len(existing_student_numbers)} students from specified list in term {term}"
    )
    click.echo(f"Completed: {len(completed_students)}")
    click.echo(f"Failed (will retry): {len(failed_students)}")
    click.echo(f"Remaining: {len(remaining_students)}")
    ...
    if not all_students_to_process:
        click.secho("All specified students have been processed!", fg="green")
        return
    ...
    try:
        for i, std_no in enumerate(all_students_to_process):
            ...
            processed_count = len(completed_students) + i
            progress_percent = (processed_count / len(existing_student_numbers)) * 100
            ...
            status_indicator = "(RETRY)" if std_no in failed_students else ""
            click.echo(
                f"[{processed_count + 1}/{len(existing_student_numbers)}] ({progress_percent:.1f}%){time_estimate_str}"
            )
            click.echo(f"Processing student {std_no} {status_indicator}...")
        ...
        try:
            ...
            logger.error(f"Failed to save final progress to {progress_file}: {e}")

        end_time = time.time()
        total_time = end_time - start_time

        logger.info(
            f"Specific students update completed. Total time: {format_time_estimate(total_time)}"
        )
        logger.info(f"Failed updates: {len(failed_students)} students")
        logger.info(f"Students processed: {students_processed}")
        ...
        if len(completed_students) + len(failed_students) >= len(
            existing_student_numbers
        ):
            click.secho("✓ All specified students processed!", fg="green")
        else:
            remaining = (
                len(existing_student_numbers)
