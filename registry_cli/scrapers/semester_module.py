...
class SemesterModuleScraper(BaseScraper):
    ...
    def scrape(self) -> Dict[str, Any]:
        ...
        for row in rows:
            ...
            if header == "ID":
                module_data["id"] = self._extract_hidden_value(value_cell, "x_StdModuleID")
            elif header == "Semester":
                module_data["semester_id"] = self._extract_hidden_value(value_cell, "x_StdSemesterID")
                module_data["semester_code"] = value_cell.get_text(strip=True)
            elif header == "School":
                module_data["school_id"] = self._extract_hidden_value(value_cell, "x_StdSchoolID")
                module_data["school_code"] = value_cell.get_text(strip=True)
            elif header == "Program":
                module_data["program_id"] = self._extract_hidden_value(value_cell, "x_StdProgramID")
                module_data["program_name"] = value_cell.get_text(strip=True)
            elif header == "Module":
                # Extract SemesterModule ID and name
                module_data["semester_module_id"] = self._extract_hidden_value(value_cell, "x_SemModuleID")
                ...
                if select:
                    selected_option = select.find("option", selected=True)
                    if selected_option:
                        module_data["credits"] = float(selected_option.get_text(strip=True))
                else:
        ...
        # Extract module_id from hidden input
        module_id_input = soup.find("input", {"id": "x_ModuleID"})
        if module_id_input and isinstance(module_id_input, Tag) and "value" in module_id_input.attrs:
    ...
    def _extract_hidden_value(self, cell: Union[Tag, NavigableString], input_id: str) -> Optional[int]:
        ...
        """
        # Skip if cell is not a Tag
        if isinstance(cell, NavigableString):
            return None
            
        # Now we can safely use cell as a Tag
        cell_tag = cast(Tag, cell)
        hidden_input = cell_tag.find("input", {"id": input_id})
        
        if hidden_input and isinstance(hidden_input, Tag) and "value" in hidden_input.attrs:
            try:
